...
import { ActorRdfMembershipFilterTyped } from '@comunica/bus-rdf-membership-filter';
...
export class ActorRdfMembershipFilterBloom extends ActorRdfMembershipFilterTyped {
  ...
  public async run(action: IActionRdfMembershipFilter): Promise<IActorRdfMembershipFilterOutput> {
    const bits = Number.parseInt(action.properties[ActorRdfMembershipFilterBloom.MEM_BITS], 10);
    const hashes = Number.parseInt(action.properties[ActorRdfMembershipFilterBloom.MEM_HASHES], 10);
