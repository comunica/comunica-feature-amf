...
describe('ApproximateMembershipFilterLazy', () => {
  ...
  beforeEach(() => {
    mediatorRdfMembership = {
      mediate: async ({ typeUri, properties }) => {
        if (typeUri === 'UNKNOWN') {
          throw new Error('Unknown filter type ' + typeUri);
        }
        return {
          filter: {
            filter: (term) => properties['http://semweb.mmlab.be/ns/membership#filter'] === term.value,
          },
        };
      },
    };
    mediatorRdfDereference = {
      mediate: jest.fn(async () => ({
        quads: new ArrayIterator([
          quad('http://ex.org/filter', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type',
            'http://ex.org/type'),
          quad('http://ex.org/filter', 'http://semweb.mmlab.be/ns/membership#filter',
            '"abc"'),
        ]),
      })),
    };
  ...
  describe('instantiated', () => {
    it('should have an uninitialized filterInstance', () => {
      return expect(filter.filterInstance).toBe(null);
    });
    ...
    it('should error on missing types', async () => {
      ...
      filter = new ApproximateMembershipFilterLazy('http://ex.org/filter',
        mediatorRdfMembership, mediatorRdfDereference);
      return expect(filter.filter(namedNode('http://example.org/'))).rejects
